...
trait Applicative[F[_]] extends Functor[F] { self =>
  ...
  def sequence[A](fas: List[F[A]]): F[List[A]] =
    fas.foldRight(unit(List[A]())){ (cur, acc) => map2(cur, acc)(_ :: _) }

  def traverse[A,B](as: List[A])(f: A => F[B]): F[List[B]] =
    as.foldRight(unit(List[B]())){ (cur, acc) => map2(f(cur), acc)(_ :: _) }
  ...
  def map4[A, B, C, D, E](fa: F[A], fb: F[B], fc: F[C], fd: F[D])(f: (A, B, C, D) => E): F[E] =
    apply(map3(fa, fb, fc)((a, b, c) => f(a, b, c, _)))(fd)
  ...
  def sequenceMap[K,V](ofa: Map[K,F[V]]): F[Map[K,V]] =
    ofa.foldLeft(unit(Map[K, V]()))((acc, cur) => map2(acc, cur._2)((a, c) => a + (cur._1 -> c)))
}
...
object Traverse {
  val listTraverse = new Traverse[List] {
    override def traverse[G[_]: Applicative, A, B](fa: List[A])(f: A => G[B]): G[List[B]] = {
      ...
      fa.foldRight(G.unit(List[B]()))((cur, acc) => G.map2(f(cur), acc)(_ :: _))
    }
  }
  ...
  val optionTraverse = new Traverse[Option] {
    override def traverse[G[_]: Applicative, A, B](fa: Option[A])(f: A => G[B]): G[Option[B]] = {
      ...
      fa match {
        ...
        case Some(value) => G.map(f(value))(Option.apply)
      }
    }
  }
  ...
  val treeTraverse = new Traverse[Tree] {
    override def traverse[G[_]: Applicative, A, B](fa: Tree[A])(f: A => G[B]): G[Tree[B]] = {
      val G: Applicative[G] = implicitly[Applicative[G]]
      G.map2(f(fa.head), listTraverse.traverse(fa.tail)(traverse(_)(f)))(Tree.apply)
    }
  }
}
